...
# scraper.sh: Scrapes media at yande.re,konachan.com,danbooru.donmai.us,safebooru.org,waifu.im,rule34.xxx
...
WAIFUIM_API="https://api.waifu.im/images?IsNsfw=All&IncludedTags="
...
if [ -z "$1" ] || [ -z "$2" ]; then
 # No tag and booru specified.
  ...
  echo "Booru options: yandere, konachan, danbooru, safebooru, waifuim, rule34"
  exit 1
fi
...
case "$BOORU" in
  yandere)
    URL="${YANDERE_API}${TAG}${HARDCODED_EXCLUDED_TAG}"
    DIR="yandere_$TAGS"
    JQ_FILTER='.[].jpeg_url'
    ;;
  konachan)
    URL="${KONACHAN_API}${TAG}"
    DIR="konachan_$TAGS"
    JQ_FILTER='.[].jpeg_url'
    ;;
  danbooru)
  ...
  *)
    echo "Invalid booru."
    echo "Valid options: yandere, konachan, danbooru, safebooru, waifuim, rule34"
    exit 1
    ;;
esac
